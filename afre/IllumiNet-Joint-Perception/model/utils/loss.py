...
class DetectionLoss(BaseLoss):
    ...
    def decode_bbox(self, anchor_points, pred_dist):
            """
            Decodes predicted object bounding box coordinates from anchor points and 
            predicted box distribution.
            """
            b, a, c = pred_dist.shape  # batch, anchors, channels
            
            # --- FIX: Ensure projection tensor is on the same device as predictions ---
            ...
            # Reshape to (batch, anchors, 4, reg_max) -> softmax -> multiply by projection
            pred_boxes = pred_dist.view(b, a, 4, c // 4).softmax(dim=3) @ self.proj
            
            # Convert distance to bbox (xyxy format)
            return dist2bbox(pred_boxes, anchor_points, xywh=False, dim=-1)
...
class MTLLoss(BaseLoss):
    ...
    def compute_loss(self, batch, preds):
        ...
        img = batch['img'].to(llie_res_map.device)
        gt_img = batch['gt_img'].to(llie_res_map.device) # Target (Normal Light)
