...
from training.callbacks import set_task_specific_params, CosineAnnealingWarmupLR, CompositeScoreCallback
...
class ModelTrainer:
    def __init__(self, 
        ...
        # --- Metrics & Saving ---
        self.saver = CompositeScoreCallback(model_name=self.name)
        self.history = []

    def _setup_optimizer(self, lr, decay=1e-4):
    ...
    def validate(self):
        ...
        with torch.no_grad():
            for batch_data in pbar:
                ...
                with torch.cuda.amp.autocast():
                    total_loss, det_loss, enhance_loss, det_out, llie_out, _ = compute_total_loss(
                        self.model, low_imgs, high_imgs, det_targets, 
                        self.det_loss_fn, self.enhance_loss_fn, inference=True
                    )
                    ...
                    for b in range(B):
                        ...
                        boxes = p[:, :4]                     # already xyxy, absolute
                        cls_scores, labels = p[:, 4:].max(dim=1)

                        mask = cls_scores > 0.25
                        boxes, scores, labels = boxes[mask], cls_scores[mask], labels[mask]
                        ...
                        gt_boxes = det_targets['bboxes'][gt_mask]   # normalized xywh
                        gt_labels = det_targets['cls'][gt_mask]

                        gt_boxes = xywh_norm_to_xyxy_abs(gt_boxes, img_size=640)
        ...
        return {
            ...
            # 'val_iou': float(avg_iou),
            # 'val_precision': float(m_pre),
            # 'val_recall': float(m_rec),
            # 'val_mAP50': float(map50)
            'val_prec_prox' : self.map_results['map'].item(), 
            'val_mAP50': self.map_results['map_50'].item(),
            'val_mAP': self.map_results['map'].item(),
            'val_recall': self.map_results['mar_100'].item(),
        }
