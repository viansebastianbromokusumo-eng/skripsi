...
from losses.total_loss import compute_total_loss, dynamic_total_loss
...
class ModelTrainer:
    def __init__(self, 
        ...
        if self.device_type == 'cuda':
            self.scaler = torch.amp.GradScaler(device=self.device_type, init_scale=1024.0)
        else:
            self.scaler = None
    ...
    def _train_epoch(self, epoch, total_epochs):
        ...
        for batch_idx, batch_data in enumerate(pbar):
            ...
            if torch.isnan(total_loss):
                print(f"\n[FATAL] NaN detected at Epoch {epoch}, Batch {batch_idx}!")
                print(f"Det Loss: {det_loss.item()} | Enhance Loss: {enhance_loss.item()}")
                
                # Save the exact tensors that broke the model so we can load them and test
                torch.save({
                    'low_imgs': low_imgs, 
                    'high_imgs': high_imgs, 
                    'det_targets': det_targets
                }, "nan_killer_tensors.pt")
                
                raise ValueError("Stopping training immediately to prevent NaN cascade.")
            # --------------------
...
class DynamicLossWeighting(nn.Module):
    ...
    def forward(self, loss_det, loss_enh):
        # --- THE HARD GUARDRAIL ---
        # Force Detection weight to NEVER drop below 1.0 (you can even change this to 2.0 or 5.0)
        precision_det = torch.clamp(torch.exp(-self.log_vars[0]), min=1.0)
        weighted_loss_det = precision_det * loss_det + self.log_vars[0]

        # Let Enhancement do whatever it wants to balance itself
        precision_enh = torch.exp(-self.log_vars[1])
        weighted_loss_enh = precision_enh * loss_enh + self.log_vars[1]

        total_loss = weighted_loss_det + weighted_loss_enh
        
        return total_loss, precision_det.item(), precision_enh.item()
    

class DynamicWeightingModelTrainer:
    def __init__(self, 
                 model, 
                 train_loader, 
                 val_loader, 
        ...
        # --- Optimization Setup ---
        self.dynamic_weighter = DynamicLossWeighting(num_tasks=2).to(self.device)
        self.optimizer = self._setup_optimizer(initial_lr)
    ...
    def _setup_optimizer(self, lr, decay=1e-10):
        ...
        param_groups = [
            ...
            {'params': neck_head_params, 'lr': 1e-4, 'weight_decay': decay},
            {'params': decoder_params, 'lr': 1e-3, 'weight_decay': decay},
            {'params': self.dynamic_weighter.parameters(), 'lr': 1e-3}
        ]
        return AdamW(param_groups, lr=lr)
    ...
    def _train_epoch(self, epoch, total_epochs):
        ...
        for batch_idx, batch_data in enumerate(pbar):
            ...
            # 2. Forward pass with AMP
            with torch.autocast(device_type=self.device.type, enabled=(self.device_type == 'cuda')):
                total_loss, det_loss, enhance_loss, _, _, _, prec_det, prec_enh = dynamic_total_loss(
                    self.model, low_imgs, high_imgs, det_targets, 
                    self.det_loss_fn, self.enhance_loss_fn, self.dynamic_weighter
                )
                
            # Tripwire for broken tensors
            if torch.isnan(total_loss):
                print(f"\n[FATAL] NaN detected at Epoch {epoch}, Batch {batch_idx}!")
                print(f"Det Loss: {det_loss.item()} | Enhance Loss: {enhance_loss.item()}")
            ...
            if self.scaler is not None:
                ...
                self.scaler.unscale_(self.optimizer)
                torch.nn.utils.clip_grad_norm_(self.model.parameters(), max_norm=10.0)
                torch.nn.utils.clip_grad_norm_(self.dynamic_weighter.parameters(), max_norm=1.0)
                self.scaler.step(self.optimizer)
                self.scaler.update()
            else:
                total_loss.backward()
            ...
            total_det_loss += det_loss.item()
            total_enhance_loss += enhance_loss.item()
            avg_prec_det += prec_det
            avg_prec_enh += prec_enh
        ...
        return {
            ...
            'train_loss_det': total_det_loss / n_batches,
            'train_loss_enh': total_enhance_loss / n_batches,
            'train_w_det': avg_prec_det / n_batches, 
            'train_w_enh': avg_prec_enh / n_batches, 
            'epoch_duration': time.time() - start_time
        }
    ...
    def validate(self):
        ...
        with torch.no_grad():
            for idx, batch_data in enumerate(pbar):
                ...
                with torch.autocast(device_type=self.device.type, enabled=(self.device_type == 'cuda')):
                    total_loss, det_loss, enhance_loss, _, llie_out, _, _, _ = dynamic_total_loss(
                        self.model, low_imgs, high_imgs, det_targets, 
                        self.det_loss_fn, self.enhance_loss_fn, self.dynamic_weighter
                    )
    ...
    def fit(self):
        ...
        for epoch in range(epochs):
            ...
            print(f"\n--- Epoch {epoch} Summary ({train_metrics['epoch_duration']:.2f}s) ---")
            print(f"Train Loss: {train_metrics['train_loss_total']:.4f} | Val Loss: {val_metrics['val_loss_total']:.4f}")
            print(f"Learned Weights -> Det: {train_metrics['train_w_det']:.4f} | Enh: {train_metrics['train_w_enh']:.4f}")
            print(f"Enhancement: PSNR={val_metrics['val_psnr']:.2f} | SSIM={val_metrics['val_ssim']:.3f}")
            print(f"Detection: mAP@0.5={val_metrics['val_mAP50']:.3f}")
            print("-------------------------------------------\n")
...
class EnhancementTrainer:
    ...
    def _setup_optimizer(self, lr, decay=1e-10):
        """Internal helper to set learning rates just for Backbone + Decoder."""
        backbone_params = []
        decoder_params = []
        ...
        # Using 1e-4 for pretrained backbone, 1e-3 for the new decoder
        param_groups = [
            {'params': backbone_params, 'lr': 1e-4, 'weight_decay': decay},
            {'params': decoder_params, 'lr': 1e-3, 'weight_decay': decay}
        ]
    ...
    def _train_epoch(self, epoch, total_epochs):
        ...
        for batch_idx, batch_data in enumerate(pbar):
            ...
            # 2. Forward pass with AMP
            with torch.autocast(device_type=self.device.type, enabled=(self.device_type == 'cuda')):
                # Model should just output the enhanced image
                enhanced_imgs, _ = self.model(low_imgs) 
                loss = self.enhance_loss_fn(enhanced_imgs, high_imgs)
                
            if torch.isnan(loss):
                raise ValueError(f"NaN detected at Epoch {epoch}, Batch {batch_idx}! Stopping training.")
            ...
            pbar.set_postfix({
                'Loss': f'{loss.item():.4f}',
                'LR_Back': f'{self.optimizer.param_groups[0]["lr"]:.2e}',
                'LR_Dec': f'{self.optimizer.param_groups[1]["lr"]:.2e}'
            })

        return {
            'train_loss': total_epoch_loss / len(self.train_loader),
            'epoch_duration': time.time() - start_time
        }
    ...
    def validate(self):
        ...
        with torch.no_grad():
            for idx, batch_data in enumerate(pbar):
                ...
                # Metrics
                total_psnr += psnr(enhanced_imgs, high_imgs).item()
                for i in range(B):
                    total_ssim += ssim_metric(enhanced_imgs[i], high_imgs[i])
                    total_ssim_count += 1
        ...
        n_batches = len(self.val_loader)
        return {
            'val_loss': total_val_loss / n_batches,
            'val_psnr': total_psnr / n_batches,
            'val_ssim': total_ssim / total_ssim_count if total_ssim_count > 0 else 0
        }
        
    def fit(self):
        print(f"Starting Enhancement Training on {self.device} for {self.epochs} epochs...")
        ...
        for epoch in range(self.epochs):
            train_metrics = self._train_epoch(epoch, self.epochs)
            val_metrics = self.validate()
            ...
            # Checkpoint: Save best model based on PSNR
            if val_metrics['val_psnr'] > self.best_psnr:
                self.best_psnr = val_metrics['val_psnr']
                best_path = f'best_{self.name}_psnr.pt'
                torch.save(self.model.state_dict(), best_path)
                print(f"*** New Best PSNR: {self.best_psnr:.2f} -> Saved {best_path} ***")
        ...
        # Final Save
        final_path = f'final_{self.name}_epoch{self.epochs}.pt'
        torch.save(self.model.state_dict(), final_path)
        
        # Save CSV History
        pd.DataFrame(self.history).to_csv(f'training_hist_{self.name}.csv', index=False)
...
class YoloTrainer:
    def __init__(self, 
        ...
        # Torchmetrics mAP calculator
        from torchmetrics.detection.mean_ap import MeanAveragePrecision
        self.map_metric = MeanAveragePrecision(iou_thresholds=[0.5], class_metrics=False).to(self.device)
        self.model.to(self.device)
        ...
        # --- Metrics & Saving ---
        self.saver = BestMAPCallback(model_name=self.name) # Assuming you still have this class
        self.history = []

    def _setup_optimizer(self, decay=1e-10): # Note: standard YOLO decay is higher than 1e-10
        """Internal helper to set learning rates just for YOLO Backbone + Neck + Head."""
        backbone_params = []
        neck_head_params = []
        ...
        param_groups = [
            {'params': backbone_params, 'lr': 1e-4, 'weight_decay': decay}, # Backbone learns slower
            {'params': neck_head_params, 'lr': 1e-4, 'weight_decay': decay}
        ]
    ...
    def _train_epoch(self, epoch, total_epochs):
        ...
        for batch_idx, batch_data in enumerate(pbar):
            # 1. Grab the images_low (Which are now your ENHANCED images from Stage 1!)
            inputs = batch_data['images_low'].to(self.device)
            ...
            # 2. Forward pass with AMP
            with torch.autocast(device_type=self.device.type, enabled=(self.device_type == 'cuda')):
                # Forward pass through YOLO clone 
                # (adjust if your clone returns something other than det_out)
                det_out = self.model(inputs, inference=False) 
                
                # Calculate strictly detection loss
                loss = self.det_loss_fn(det_targets, det_out)
            ...
            total_epoch_loss += loss.item()
            pbar.set_postfix({
                'DLoss': f'{loss.item():.4f}',
                'LR_BB': f'{self.optimizer.param_groups[0]["lr"]:.2e}',
                'LR_NH': f'{self.optimizer.param_groups[1]["lr"]:.2e}'
            })

        return {
            'train_loss_det': total_epoch_loss / len(self.train_loader),
            'epoch_duration': time.time() - start_time
        }
    ...
    def validate(self):
        ...
        with torch.no_grad():
            for idx, batch_data in enumerate(pbar):
                ...
                with torch.autocast(device_type=self.device.type, enabled=(self.device_type == 'cuda')):
                    # Forward for loss
                    det_out_train = self.model(inputs, inference=False)
                    loss = self.det_loss_fn(det_targets, det_out_train)
                    total_val_loss += loss.item()
                    
                    # Forward for metrics
                    det_out_inf = self.model(inputs, inference=True)
                    
                    preds, targets = [], []

                    for b in range(B):
                        # -------- Predictions --------
                        p = det_out_inf[b].permute(1, 0)  
                        boxes_xywh = p[:, :4]         
                        cls_scores, labels = p[:, 4:].max(dim=1)
        ...
        return {
            'val_loss_det': total_val_loss / n_batches,
            'val_mAP50': map_results['map_50'].item(),
            'val_mAP': map_results['map'].item(),
        }
        
    def fit(self):
        print(f"Starting YOLO 2-Stage Training on {self.device} for {self.epochs} epochs...")
        ...
        for epoch in range(self.epochs):
            ...
            epoch_log = {
                ...
                **val_metrics,
                ...
                'lr_nh': self.optimizer.param_groups[1]['lr']
            }
            self.history.append(epoch_log)
            ...
            # Print Summary
            print(f"\n--- Epoch {epoch} Summary ({train_metrics['epoch_duration']:.2f}s) ---")
            print(f"Train Loss: {train_metrics['train_loss_det']:.4f} | Val Loss: {val_metrics['val_loss_det']:.4f}")
            print(f"Detection: mAP@0.5={val_metrics['val_mAP50']:.3f} | mAP={val_metrics['val_mAP']:.3f}")
            print("-------------------------------------------\n")
