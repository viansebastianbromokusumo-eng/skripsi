...
from modules.decoder import LowLightEnhancementDecoder, LowLightEnhancementDecoder_Scale
...
class IllumiNet_b5_ARB_Scale(nn.Module): 
    def __init__(self, back_use_arb, dec_arb):
        super().__init__()
        print('IllumiNet with Scaler Initialized')
        print('Backbone ARB: ', back_use_arb)
        print('Decoder ARB', dec_arb)
        self.backbone = BackBone_5_ARB(version='nano', use_arb=back_use_arb)
        self.decoder = LowLightEnhancementDecoder_Scale(base_channels=64, use_arb=dec_arb)
        self.neck = Neck(version='nano')
        self.head = Head_2(version='nano')
...
class YOLOv8_Clone(nn.Module): 
    def __init__(self, back_use_arb=False):
        super().__init__()
        self.backbone = BackBone_5_ARB(version='nano', use_arb=back_use_arb)
        self.neck = Neck(version='nano')
        self.head = Head_2(version='nano')
...
class Enh_Dec_Net(nn.Module): 
    ...
    def forward(self, x): 
        stem, dark2, dark3, _, _ = self.backbone(x)
        
        llie_out, llie_residuals = self.decoder(in_img=x, stem=stem, dark2=dark2, dark3=dark3)

        return llie_out, llie_residuals
