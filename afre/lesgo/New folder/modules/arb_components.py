...
class CAU(nn.Module):
    ...
    def forward(self, F_in):
        # FIX: Temporarily cast to float32 to prevent summation overflow in FP16
        orig_dtype = F_in.dtype
        F_caa = F.avg_pool2d(F_in.float(), F_in.size()[2:]).to(orig_dtype)
        
        # Max pool doesn't sum, so it is safe to leave as-is
...
class SAU(nn.Module):
    ...
    def forward(self, F_in):
        # FIX: Temporarily cast to float32 to prevent summation overflow in FP16
        orig_dtype = F_in.dtype
        F_saa = torch.mean(F_in.float(), dim=1, keepdim=True).to(orig_dtype)
...
class ARB_Refined(nn.Module):
    ...
    def forward(self, x_in):
        ...
        # 2. Parallel Branches (These are healthy!)
        F_nc = self.NCU(F_in)
        F_ca = self.CAU(F_in)
        F_sa = self.SAU(F_in)

        # 3. Strict Averaged Fusion (Prevents magnitude tripling)
        fused_features = (F_nc + F_ca + F_sa) / 3.0

        # 4. Post-Fusion Convolution
        F_mid = self.conv_post_fusion(fused_features)

        # 5. Final Path Convolution
        F_final_conv = self.conv_final(F_mid)
        
        # --- THE FIX: FP16 Variance Clamp ---
        # Forbid the tensor from exceeding +/- 100 so BatchNorm squaring NEVER exceeds 65,504
        F_final_conv = torch.clamp(F_final_conv, min=-100.0, max=100.0)
        
        # Now BatchNorm is 100% safe
        F_arb_path = self.BN(F_final_conv)

        # 6. Final Residual Connection (Averaged to protect downstream YOLO layers)
        F_out = (F_arb_path + F_in) / 2.0
