...
class LoLiStreetDataset_Curated(Dataset):
    """
    Dataset that uses a curated CSV to map dense, moderate, and light 
    images of the same scene to a single chosen Ground Truth file.
    """
    def __init__(self, csv_path: str, high_dir: str, low_dir: str, 
                 low_gt_dir: str, high_gt_dir: str, 
                 img_size: tuple[int, int] = (640, 640), transform=None):
        super().__init__()
        self.img_size = img_size
        self.transform = transform if transform else T.ToTensor()
        ...
        # Read the curated manifest
        try:
            df = pd.read_csv(csv_path)
        except FileNotFoundError:
            raise FileNotFoundError(f"[Error] CSV not found at: {csv_path}")

        print(f"Parsing CSV... Found {len(df)} curated scenes.")

        for _, row in df.iterrows():
            orig_filename = row['filename']  # e.g., 'dense_10.jpg'
            chosen_gt = row['chosen_gt']     # 'low' or 'high'

            # 1. Extract the number from the filename
            # This looks for an underscore, followed by digits, ending in .jpg
            match = re.search(r'_(\d+)\.jpg', orig_filename, re.IGNORECASE)
            if not match:
                print(f"Skipping {orig_filename}: Could not extract ID.")
                continue
            
            img_num = match.group(1) # Extracts '10' from 'dense_10.jpg'

            # 2. Fetch the detection annotation based on chosen low/high dir
            # The label name matches the CSV filename but with .txt
            label_name = orig_filename.replace('.jpg', '.txt') 
            
            if chosen_gt == 'low':
                target_gt_path = os.path.join(low_gt_dir, label_name)
            else:
                target_gt_path = os.path.join(high_gt_dir, label_name)

            if not os.path.exists(target_gt_path):
                # Optional: Print warning if the GT text file is completely missing
                continue

            # 3 & 4. Fetch all 3 image variants and point them to the same GT
            illum_types = ['dense', 'moderate', 'light']
            
            for illum in illum_types:
                variant_filename = f"{illum}_{img_num}.jpg"
                
                low_img_path = os.path.join(low_dir, variant_filename)
                high_img_path = os.path.join(high_dir, variant_filename)

                # Verify both the low and high images exist for this variant
                if os.path.exists(low_img_path) and os.path.exists(high_img_path):
                    self.low_images.append(low_img_path)
                    self.high_images.append(high_img_path)
                    
                    # MAGIC HAPPENS HERE: All three variants get the exact same target_gt_path
                    self.labels_files.append(target_gt_path) 

        print(f"Dataset Built! Expanded to {len(self.high_images)} total image-label pairs.")
